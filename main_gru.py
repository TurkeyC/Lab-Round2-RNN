...
def train():
    ...
    os.makedirs("./model/gru_model", exist_ok=True)
    os.makedirs("./record", exist_ok=True)
    Net = GRU(p.input_size, p.hidden_size, p.num_layer, p.num_class, p.device).to(
        p.device
    )
    ...
    for epoch in range(p.num_epochs):
        ...
        for i, (mfcc, label, mfcc_size) in enumerate(train_data_loader):
            mfcc = mfcc.squeeze(1).transpose(1, 2).to(p.device, non_blocking=pin_memory)
        ...
        with torch.no_grad():
            ...
            for i, (mfcc, label, mfcc_size) in enumerate(test_data_loader):
                mfcc = mfcc.squeeze(1).transpose(1, 2).to(
                    p.device, non_blocking=pin_memory
                )
                label = label.to(p.device, non_blocking=pin_memory)
                output = Net(mfcc)
                loss = criterion(output, label)
        ...
        torch.save(Net, f"./model/gru_model/gru_{epoch}.pth")

    df = pd.DataFrame(logger)
    df.to_csv("./record/log_gru.csv", index=False)
