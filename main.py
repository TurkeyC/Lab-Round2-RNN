...
Net = LSTM(p.input_size, p.hidden_size, p.num_layer, p.num_class, p.device).to(p.device)
...
for epoch in range(p.num_epochs):
    ...
    for i, (mfcc, label, mfcc_size) in enumerate(train_data_loader):
        mfcc = mfcc.squeeze().transpose(1, 2).to(p.device)
    ...
    torch.save(Net, f"./model/model_{epoch}.pth")


df = pd.DataFrame(logger)
df.to_csv("./log8.csv", index=False)
